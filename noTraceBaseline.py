...
def serializeProgram(l):
    return (
        serializeLine(l)
        + ([] if l.boundary == None else ["if"] + serializeProgram(l.boundary) + ["}"])
...
def parseOutput(l):
    ...
    def parseAtomic(l):
        ...
        if k == "line":
            ...
            s = get(l)
            return Primitive(
                k, x1, y1, x2, y2, "arrow = True" == a, "solid = True" == s
            )
        if k == "for":
            v = get(l)
            b = parseLinear(l)
...
class CaptionEncoder(nn.Module):
    def __init__(self):
        ...
        self.squareFilters = nn.Conv2d(
            1, squareFilters, kernelSizes[0], padding=kernelSizes[0] / 2
        )
        self.verticalFilters = nn.Conv2d(
            1,
            rectangularFilters,
            (kernelSizes[0] / 2 - 1, kernelSizes[0] * 2 - 1),
            padding=(kernelSizes[0] / 4 - 1, kernelSizes[0] - 1),
        )
        self.horizontalFilters = nn.Conv2d(
            1,
            rectangularFilters,
            (kernelSizes[0] * 2 - 1, kernelSizes[0] / 2 - 1),
            padding=(kernelSizes[0] - 1, kernelSizes[0] / 4 - 1),
        )
        self.laterStages = nn.Sequential(
            nn.ReLU(),
            nn.MaxPool2d(poolSizes[0], poolStrides[0], padding=poolSizes[0] / 2 - 1),
            nn.Conv2d(
                squareFilters + 2 * rectangularFilters,
                numberOfFilters[0],
                kernelSizes[1],
...
class CaptionDecoder(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        expandedFeatures = features.unsqueeze(1).expand(
            features.size(0), e.size(1), features.size(1)
        )
    ...
    def buildCaptions(self, tokens):
        ...
        tokens = [
            np.concatenate((p, np.zeros(maximumSize + 1 - len(p), dtype=np.int)))
            for p in tokens
        ]
...
class NoTrace(nn.Module):
    ...
    def sampleMany(self, sequence, duration):
        image = variable(
            np.array([sequence.draw()], dtype=np.float32), volatile=True
        ).unsqueeze(1)
    ...
    def load(self, path):
        if os.path.isfile(path):
            if not GPU:
                stuff = torch.load(path, map_location=lambda s, l: s)
            else:
    ...
    def __init__(self, p):
        try:
            self.tokens = np.array(
                [symbolToIndex["START"]]
                + [symbolToIndex[s] for s in serializeProgram(p)]
                + [symbolToIndex["END"]]
            )
        except KeyError:
...
def loadTrainingData(n):
    ...
    alternatives = [
        "/scratch/ellisk/randomlyGeneratedPrograms.p",
...
if __name__ == "__main__":
    ...
    if "test" in sys.argv:
        ...
        results.sort(
            key=lambda z: (z["spec"] - target, z["program"].totalCost(), z["time"])
        )
        if len(results) > 0:
            z = results[0]
            print("Best program:")
            ...
            saveMatrixAsImage(
                255 * np.concatenate((1 - target.draw(), z["spec"].draw()), axis=0),
    ...
    if "statistics" in sys.argv:
        ...
        for n in range(100):
            ...
            try:
                ...
                results.append(None)
        times = [r["time"] if r else float("inf") for r in results]
        medianTime = sorted(times)[len(times) / 2]
        print("Median time", medianTime)
        successful = sum(
            r is not None and (r["spec"] - g) == 0 for r, g in zip(results, gt)
        )
        print(
            "# times that we got a program which was consistent with the data",
            successful,
        )
        os.exit(0)
    ...
    while True:
        ...
        while start < N:
            ...
            if batchIndex % 50 == 0:
                print(
                    "Batch [%d/%d], LOSS = %s" % (batchIndex, batchesPerLoop, L.data[0])
                )
