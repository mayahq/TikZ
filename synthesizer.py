...
class SynthesisResult:
    ...
    def exportToFile(self, f):
        with open(f, "w") as handle:
            handle.write(
                "Found the following cost-%d program after %f seconds:\n%s"
                % (self.cost, self.time, self.program.pretty())
            )
...
class SynthesisJob:
    ...
    def __str__(self):
        return (
            "SynthesisJob(%s,incremental = %s,maximumD = %s,loops = %s,reflects = %s,prior = %s)"
            % (
                self.originalDrawing,
                self.incremental,
                self.maximumDepth,
                self.canLoop,
                self.canReflect,
    ...
    def subsumes(self, other):
        ...
        if self.incremental:
            return False  # ??? need to understand this better...
        return (
            self.incremental == other.incremental
            and self.maximumDepth >= other.maximumDepth
            and self.canLoop >= other.canLoop
            and self.canReflect >= other.canReflect
        )  # and not self.incremental

    def execute(self, timeout=60, parallelSolving=1):
        if self.incremental:
            return self.executeIncrementally(
                timeout=timeout, parallelSolving=parallelSolving
            )
        else:
            return self.executeJoint(timeout=timeout, parallelSolving=parallelSolving)

    def executeJoint(self, timeout=60, parallelSolving=1):
        startTime = time.time()
        result = synthesizeProgram(
            self.parse,
            self.usePrior,
    ...
    def executeIncrementally(self, timeout=60, parallelSolving=1):
        ...
        for l in self.parse.lines:
            if isinstance(l, Circle):
                jobs["Circle"] = jobs.get("Circle", []) + [l]
            elif isinstance(l, Rectangle):
                jobs["Rectangle"] = jobs.get("Rectangle", []) + [l]
            elif isinstance(l, Line):
                jobs["Line%s%s" % (l.solid, l.arrow)] = jobs.get(
                    "Line%s%s" % (l.solid, l.arrow), []
                ) + [l]
            else:
                assert False
        ...
        jobOrdering = sorted(
            list(jobs.keys()), key=lambda stuff: (len(stuff) < 3, len(stuff))
        )
        ...
        for k in jobOrdering:
            ...
            jobResults[k] = synthesizeProgram(
                Sequence(jobs[k]),
                self.usePrior,
        ...
        try:
            gluedTogether = Block(
                [
                    x
                    for _, result in list(jobResults.values())
            ...
            e = sys.exc_info()[0]
            print(
                " [-] Problem parsing or optimizing %s: %s" % (self.originalDrawing, e)
            )
            optimalProgram = None
            optimalCost = None
...
def parallelExecute(jobs):
    if arguments.cores == 1:
        ...
        return Pool(arguments.cores).map(
            lambda j: invokeExecuteMethod(j, timeout=arguments.timeout), jobs
        )
...
def expertSynthesisJobs(k):
    ...
    for j in range(100):
        ...
        for p in loadTopParticles(particleDirectory, k):
            newJobs.append(
                SynthesisJob(
                    p.sequence(), originalDrawing, usePrior=not arguments.noPrior
                )
            )
        # but we don't care about synthesizing if there wasn't a ground truth in them
        if any(
            [newJob.parse == getGroundTruthParse(originalDrawing) for newJob in newJobs]
        ):
            jobs += newJobs
...
def synthesizeTopK(k):
    ...
    if k == 0:
        for k in groundTruthSequence:
            ...
            if all([not (r.parse == sequence) for r in results]):
...
def makePolicyTrainingData():
    ...
    print(
        " [+] Constructed %d job objects for the purpose of training a policy"
        % (len(jobs))
    )
...
def viewSynthesisResults(arguments):
    ...
    for expertIndex in list(range(100)):
        ...
        if relevantResults == []:
            ...
            programs = [
                r.program.fixStringParameters()
                .fixReflections(result.job.parse.canonicalTranslation())
                .removeDeadCode()
            ...
            if badPrograms:
                print(
                    " [-] WARNING: Got %d programs that are inconsistent with ground truth"
                    % (len(badPrograms))
                )
            if False:
                for program in programs:
                    ...
                    if not (prediction == actual):
                        ...
                        print(
                            set(map(str, actual.lines))
                            ^ set(map(str, prediction.lines))
                        )
                        assert False
        ...
        if arguments.extrapolate:
            ...
            if extrapolations:
                ...
                print(
                    "Saving extrapolation column to",
                    "extrapolations/expert-%d-extrapolation.png" % expertIndex,
                )
                saveMatrixAsImage(
                    a, "extrapolations/expert-%d-extrapolation.png" % expertIndex
                )
...
""" % (
        ...
        if False and extrapolations != [] and arguments.extrapolate:
            ...
            # print e
            rightEntryOfTable = (
                "\\includegraphics[width = 5cm]{../TikZ/extrapolations/expert-%d-extrapolation.png}"
                % expertIndex
            )
    ...
    if arguments.latex:
        latex = "%s" % ("\\\\\n".join(latex))
        name = (
            "extrapolations.tex" if arguments.extrapolate else "synthesizerOutputs.tex"
        )
    ...
    if arguments.extrapolate:
        ...
        bigMatrix = np.zeros(
            (
                max([m.shape[0] for m in extrapolationMatrix]),
...
def rankUsingPrograms():
    ...
    def featuresOfParticle(p):
        ...
        if r != None and r.cost != None and r.source != None:
            programFeatures = mergeDictionaries(
                {"failure": 0.0}, parseSketchOutput(r.source).features()
            )
        else:
    ...
    topParticles = [
        loadTopParticles("drawings/expert-%d-parses" % j, k) for j in range(100)
    ]
    learningProblems = []
    for j, ps in enumerate(topParticles):
        ...
        if positives != [] and negatives != []:
            learningProblems.append(
                (
                    list(map(featuresOfParticle, positives)),
    ...
    learningProblems = [
        (
            list(map(dictionaryToVector, positives)),
            list(map(dictionaryToVector, negatives)),
        )
...
def induceAbstractions():
    ...
    for i, j, a, e in abstractions:
        ...
        if "for " in p:
            ...
            allowUnattached = (
                firstProgram.haveUnattachedLines()
            ...
            while len(samples) < desiredNumberOfSamples and samplingAttempts < 10000:
                ...
                if (
                    ...
                    try:
                        ...
                        pass
            samples += [np.zeros((256, 256)) + 0.5] * (
                desiredNumberOfSamples - len(samples)
            )
            ...
            print(j)
            showImage(
                np.concatenate([firstProgram.draw(), secondProgram.draw()], axis=1)
            )
            abstractionMatrix.append(np.concatenate(samples, axis=1))
    # .showImage(np.concatenate(abstractionMatrix,axis = 0),)
    saveMatrixAsImage(
        255 * np.concatenate(abstractionMatrix, axis=0), "abstractions.png"
    )
...
def analyzeSynthesisTime():
    ...
    for r in results:
        ...
        if isinstance(r.time, list):
            times.append(sum(r.time))
        else:
            times.append(r.time)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Synthesis of high-level code from low-level parses"
    )
    parser.add_argument("-f", "--file", default=None)
    parser.add_argument("-m", "--cores", default=1, type=int)
    parser.add_argument("--parallelSolving", default=1, type=int)
    parser.add_argument(
        "-n", "--name", default="groundTruthSynthesisResults.p", type=str
    )
    ...
    if arguments.view:
        ...
        if "drawings/expert-%s.png" % (arguments.file) in groundTruthSequence:
            j = SynthesisJob(
                groundTruthSequence["drawings/expert-%s.png" % (arguments.file)],
                "",
            ...
            j = SynthesisJob(
                pickle.load(open(arguments.file, "rb")).program,
                "",
                usePrior=not arguments.noPrior,
                incremental=arguments.incremental,
            )
            print(j)
            r = j.execute(
                timeout=arguments.timeout, parallelSolving=arguments.parallelSolving
            )
