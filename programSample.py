...
class SampleEnvironment:
    def __init__(self, freeVariables, rx=None, ry=None, rl=None):
        ...
        self.rx = (
            rx
            if rx != None
            else ChineseRestaurant(
                2.0, lambda: random.choice([-4, -3, -2, -1, 1, 2, 3, 4])
            )
        )
        self.ry = (
            ry
    ...
    def deepCopy(self):
        return SampleEnvironment(
            self.freeVariables, self.rx.copy(), self.ry.copy(), self.rl.copy()
        )


def sampleLinearExpression(coefficientRestaurant, freeVariables):
    if freeVariables == [] or random.random() < 0.3:
        return LinearExpression(0, None, random.choice(list(range(1, 15))))

    return LinearExpression(
        coefficientRestaurant.sampleNew(),
...
def samplePoint(e):
    return (
        sampleLinearExpression(e.rx, e.freeVariables),
        sampleLinearExpression(e.ry, e.freeVariables),
    )
...
def sampleLine(e):
    while True:
        ...
        if x1 != x2 or y1 != y2:
            return Primitive(
                "line", x1, y1, x2, y2, random.random() > 0.5, random.random() > 0.5
            )
...
def samplePrimitive(e):
    q = random.random()
    if q < 0.33:
        return sampleCircle(e)
    elif q < 0.66:
        return sampleLine(e)
    else:
        return sampleRectangle(e)
...
def sampleLoop(e):
    ...
    return Loop(chr(ord("i") + len(e.freeVariables)), b, Block([]), boundary=Block([]))


def sampleReflection(e):
    return Reflection(
        random.choice(["x", "y"]), random.choice(list(range(1, 20))), Block([])
    )
...
def mutateProgram(e, p):
    ...
    if n < len(p.items) and not isinstance(p.items[n], Primitive):
        ...
        u = random.random()
        if len(e.freeVariables) <= 1 and u < 0.3:
            new = sampleLoop(e)
        elif u < 0.75:
            new = samplePrimitive(e)
        else:
            new = sampleReflection(e)
...
def mutateProgram(e, p):
    ...
    if random.random() < 0.5:
        return Loop(p.v, p.bound, mutateProgram(ep, p.body), boundary=p.boundary)
    else:
        return Loop(p.v, p.bound, p.body, boundary=mutateProgram(ep, p.boundary))
