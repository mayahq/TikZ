...
class GraphicsSearchPolicy(SearchPolicy):
    ...
    def applyChange(self, program, environment, line):
        if isinstance(program, Loop):
            return Loop(
                body=self.applyChange(program.body, environment, line),
                v=program.v,
                bound=program.bound,
            )
    ...
    def value(self, goal, program):
        ...
        if output == None:
            return -1.0
        if len(output - goal) > 0:
            return 0.0
        else:
            return 1.0 / program.cost()
    ...
    def parseLine(self, l):
        ...
        def finish(l):
            if l != []:
                raise Exception("Extra symbols in line")
...
def Oracle(l):
    for program, environment, line in Oracle(l.body):
        yield Reflection(
            axis=l.axis, coordinate=l.coordinate, body=program
        ), environment, line
...
def simpleSceneSample():
    def isolatedCircle():
        ...
        return Primitive(
            "circle", LinearExpression(0, None, x), LinearExpression(0, None, y)
        )
    ...
    primitives = [
        isolatedCircle()
        for _ in range(random.choice(list(range(MINIMUMATOMS, MAXIMUMATOMS + 1))))
    ]
    ...
    while True:
        ...
        if all(
            [x > 0 and x < 16 for j in range(loopIterations) for x in [mx * j + bx]]
        ):
            break
    while True:
        ...
        if all(
            [y > 0 and y < 16 for j in range(loopIterations) for y in [my * j + by]]
        ):
            break

    l = Loop(
        v="j",
        bound=LinearExpression(0, None, loopIterations),
        body=Block(
            [
                Primitive(
                    "circle",
