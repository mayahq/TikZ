...
def makeSyntheticData(filePrefix, sample, k=1000, offset=0):
    ...
    noLabels = [
        Sequence([l for l in p.lines if not isinstance(l, Label)]) for p in programs
    ]
    onlyLabels = [[l for l in p.lines if isinstance(l, Label)] for p in programs]
...
def canonicalOrdering(things):
    ...
    rs = sorted(rs, key=lambda r: (r.p1.x, r.p1.y))
            ...
            for (x1, y1, o1) in attachmentSets[j]:
                for (x2, y2, o2) in attachmentSets[k]:
                    ...
                    if candidate != None:
                        ...
                        if l.length() > 0 and all(
                            [not o.intersects(l) for o in objects]
                        ):
                            if isAligned:
...
def sampleCircle(objects):
    ...
    reuseProbability = len(existingRadii) / (
        len(existingRadii) + concentrationParameter
    )
    while True:
...
def sampleLine(objects, attachedLines=[]):
    concentration = 2.0
    if attachedLines != [] and random() < float(len(attachedLines)) / (
        concentration + len(attachedLines)
    ):
        (x1, y1, x2, y2) = choice(attachedLines)
        points = [AbsolutePoint((x1), (y1)), AbsolutePoint((x2), (y2))]
    elif (
        random() < 1.0
    ):  # horizontal or vertical line: diagonals only allowed as attachments
        p1 = samplePoint(objects)
        x1 = p1.x
        y1 = p1.y
    ...
    return Line(points, solid=random() > 0.1, arrow=arrow)
...
def multipleObjects(rectangles=0, lines=0, circles=0, labels=0):
    def sampler():
        objects = []
        objects = sampleWithoutIntersection(
            circles, objects, lambda: sampleCircle(objects)
        )
        objects = sampleWithoutIntersection(
            rectangles, objects, lambda: sampleRectangle(objects)
        )
        objects = sampleWithoutIntersection(
            labels, objects, lambda: sampleLabel(objects)
        )
        attachedLines = proposeAttachmentLines(objects)
        objects = sampleWithoutIntersection(
            lines, objects, lambda: sampleLine(objects, attachedLines)
        )
...
if __name__ == "__main__":
    ...
    if examplesPerBatch > 1000:
        examplesPerBatch = 1000
    ...
    if totalNumberOfExamples > 100:
        ...
        for _, startingPoint, _ in offsetsAndCounts:
            os.system(
                "cd %s/%d && tar --append --file ../../%s.tar . && cd ../.."
                % (outputName, startingPoint, outputName)
            )
        #     if totalNumberOfExamples > 100:
        ...
        print(
            "Done. You should see everything in %s.tar if you had at least 100 examples."
            % (outputName)
        )
