    ...
    def beam(self, seed, maximumLength, beamSize):
        ...
        for _ in range(maximumLength):
            ...
            for ll, sequence, h in B:
                ...
                for _ll, c in best:
                    new.append((ll + _ll, sequence + [c], h))
            if not expanded:
                break
        ...
        B = [
            (ll, sequence[1:-1])
            for ll, sequence, h in sorted(B, reverse=True)[:beamSize]
            if sequence[-1] == "END"
        ]
        return B
...
class SearchPolicy(nn.Module):
    def __init__(self, lexicon):
        ...
        self.lineEncoder = LineEncoder(
            lexicon, layers=encodeLayers, H=H, seedDimensionality=H
        )
    ...
    def environmentLogLikelihoods(self, environments, problem):
        environmentEncodings = [
            self.encodeEnvironment(e, problem) for e in environments
        ]
        environmentScores = [self.environmentScore(e) for e in environmentEncodings]
    ...
    def loss(self, example):
        ...
        environmentLoss = -self.environmentLogLikelihoods(
            example.environments, problem
        )[0]
    ...
    def makeOracleExamples(self, program, problem):
        ...
        for intermediateProgram, environment, target in self.Oracle(program):
            intermediateProblem = self.residual(
                problem, self.evaluate(intermediateProgram)
            )
    ...
    def mayBeAppliedChange(self, initialProgram, environment, line):
        try:
            return self.applyChange(initialProgram, environment, line)
        except:
            return None
    ...
    def beam(self, problem, initialProgram, size):
        ...
        environmentScores = self.environmentLogLikelihoods(
            environments, self.encodeProblem(problem).view(1, -1)
        ).data
        candidates = [
            (
                environmentScore + lineScore,
        ...
        candidates = [
            (score, candidate) for (score, candidate) in candidates if candidate != None
        ]
    ...
    def beamSearchGraph(self, problem, initialProgram, size, steps):
        ...
        for step in range(steps):
            newFrontier = []
            for f in frontier:
                for _, candidate in self.beam(
                    self.residual(problem, self.evaluate(f)), f, size
                ):
...
class PolicyTrainingExample:
    ...
    def __str__(self):
        return "PolicyTrainingExample(problem = %s, target = %s, environments = %s)" % (
            self.problem,
            self.target,
            self.environments,
        )
