...
def processHalfpage(x, index):
    ...
    scanThreshold = 7
    if index == 93:
        scanThreshold = 4  # very light here
    ...
    x = x.crop((m[1], m[0], m[1] + gridSize * 16, m[0] + gridSize * 16))
...
def processRegion(r):  # processes 1/6 of the handout
    ...
    r = r.crop((startX, startY, startX + size, startY + size))
...
def processExpert(name):
    ...
    for j in range(200):
        ...
        processHalfpage(bottom, 2 * _j + 1).save(
            "drawings/expert-%d.png" % (2 * _j + 1)
        )
