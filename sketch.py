...
def synthesizeProgram(
    ...
    for p in parse.lines:
        if isinstance(p, Circle):
            parts.append("_c(%d,%d)" % (p.center.x - x0, p.center.y - y0))
            ...
            hasRectangles = True
            parts.append(
                "_r(%d,%d,%d,%d)" % (p.p1.x - x0, p.p1.y - y0, p.p2.x - x0, p.p2.y - y0)
            )
    ...
    for c in xCoefficients:
        coefficientGenerator1 = "| " + str(c)
    coefficientGenerator2 = ""
    for c in yCoefficients:
        coefficientGenerator2 = "| " + str(c)
    ...
    if len(coefficientValidator1) == 0:
        coefficientValidator1 = "numberOfCoefficients1 == 0"
    coefficientValidator2 = " || ".join(["c == %d" % c for c in coefficientValidator2])
    if len(coefficientValidator2) == 0:
        coefficientValidator2 = "numberOfCoefficients2 == 0"

    xValidation = " || ".join(["x == %d" % x for x in set(xValidation)])
    yValidation = " || ".join(["x == %d" % x for x in set(yValidation)])

    haveThisReflectionAlready = " || ".join(
        ["(xr == %d && yr == %d)" % (xr, yr) for xr, yr in usedReflections] + ["0"]
    )
    ...
    for bound in usedLoops:
        ...
        if bound["coefficient"] != None and bound["coefficient"] != 0:
            returnValue += " + %s*environment[%d]" % (
                bound["coefficient"],
                bound["variable"],
            )
        alreadyProvidedBounds += (
            " if (n == %d && ??) { already_have_this_loop = 1; return %s; }"
            % (bound["depth"], returnValue)
        )
    # print "alreadyProvidedBounds:"
    # print alreadyProvidedBounds

    smallestPossibleLoss = 3 * len(parse.lines) + 1
    upperBoundOnLoss = " --bnd-mbits %d" % (
        min(5, int(ceil(log2(smallestPossibleLoss + 1))))
    )
...
""" % (
    ...
    os.system(
        "sketch %s --fe-timeout %d -V 10 %s 2> %s > %s"
        % (degreeOfParallelism, timeout, fd.name, outputFile, outputFile)
    )
    ...
    if (
        "Sketch front-end timed out" in output
        or "The sketch cannot be resolved" in output
    ):
        return None
    ...
    if body != None:
        ...
        fd = tempfile.NamedTemporaryFile(
            mode="w", suffix=".failure", delete=False, dir="./parsingErrors"
        )
        fd.write(output)
        fd.close()
        print(
            "WARNING: Could not parse body. Leaving the unparsable body in %s"
            % (fd.name)
        )
