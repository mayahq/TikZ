...
def beamMixture(u, v, p, lowerBound, upperBound, stepSize, k):
    def score(x):
        ...
        return lseList(
            [
                p[j] + integrateNormalDensity(lb, ub, mu=u[j], sigma=v[j] ** 0.5)
...
if __name__ == "__main__":
    ...
    hidden = tf.layers.dense(regressionInput, 15, activation=tf.nn.sigmoid)
    ...
    for _ in range(10000):
        feed = {
            regressionInput: x_data.reshape((NSAMPLE, 1)),
            regressionOutput: y_data.reshape((NSAMPLE,)),
        }
        print(s.run([loss, optimize], feed_dict=feed)[0])
        # print s.run(scalerPrediction, feed_dict = feed)
        # assert False

    (predictMeans, predictVariance, predictMixture) = s.run(
        list(mixtureOutput), feed_dict={regressionInput: x_data.reshape((NSAMPLE, 1))}
    )
    ...
    y_predicted = [
        sampleMixture(predictMeans[j], predictVariance[j], predictMixture[j])
        for j in range(NSAMPLE)
    ]
    xs = np.arange(-10, 10, 0.1)
    y_predicted = [
        beamMixture(
            predictMeans[j], predictVariance[j], predictMixture[j], -10, 10, 0.1, 1
        )[0][0]
        for j in range(NSAMPLE)
    ]
