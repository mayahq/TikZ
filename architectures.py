...
class Architecture:
    ...
    def makeModel(self, imageInput):
        ...
        horizontalKernels = tf.layers.conv2d(
            inputs=imageInput,
            filters=self.rectangularFilters,
            kernel_size=[self.kernelSizes[0] * 2, self.kernelSizes[0] / 2],
            padding="same",
            activation=tf.nn.relu,
            strides=1,
        )
        verticalKernels = tf.layers.conv2d(
            inputs=imageInput,
            filters=self.rectangularFilters,
            kernel_size=[self.kernelSizes[0] / 2, self.kernelSizes[0] * 2],
            padding="same",
        ...
        squareKernels = tf.layers.conv2d(
            inputs=imageInput,
            filters=self.squareFilters,
            kernel_size=[self.kernelSizes[0], self.kernelSizes[0]],
            padding="same",
            activation=tf.nn.relu,
            strides=1,
        )
        ...
        nextInput = c1
        for filterCount, kernelSize, poolSize, poolStride in zip(
            numberOfFilters, kernelSizes, poolSizes, poolStrides
        ):
            c1 = tf.layers.conv2d(
                inputs=nextInput,
                filters=filterCount,
            ...
            c1 = tf.layers.max_pooling2d(
                inputs=c1, pool_size=poolSize, strides=poolStride, padding="same"
            )
            nextInput = c1
        return nextInput
...
architectures = {}
architectures["original"] = Architecture(
    256,
    12,
    4,
    numberOfFilters=[10],
