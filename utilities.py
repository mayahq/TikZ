...
def loadImages(ns):
    return [loadImage(n) for n in ns]


def cacheImage(n, content):
    IMAGEBYTES[n] = content
...
def truncatedNormal(lower=None, upper=None):
    x = np.random.normal()
    if lower != None and x < lower:
        return truncatedNormal(lower=lower, upper=upper)
    if upper != None and x > upper:
        return truncatedNormal(lower=lower, upper=upper)
    return x
...
def augmentData(b):
    return perturbSaltAndPepper(perturbOffset(perturbNoisyIntensities(b)))
...
def removeBorder(x):
    while np.all(x[0, :] < 0.1):
        x = x[1:, :]
    while np.all(x[x.shape[0] - 1, :] < 0.1):
        x = x[: x.shape[0] - 2, :]
    while np.all(x[:, 0] < 0.1):
        x = x[:, 1:]
    while np.all(x[:, x.shape[1] - 1] < 0.1):
        x = x[:, : x.shape[1] - 2]
    return x
...
def invertTransformation(t):
    return np.linalg.inv(
        np.concatenate([t.reshape((2, 3)), np.array([0, 0, 1.0]).reshape((1, 3))])
    )[[0, 1], :]
...
def crossValidate(fullDataSet, folds=10, randomSeed=0, doNotPermute=False):
    if folds == 1:
        ...
        if doNotPermute:
            ...
            indices = randomlyPermuteList(list(range(n)))
        ...
        for f in range(folds):
            if f < numberOfBig:
                ending = start + bigTestSize
            else:
                ending = start + smallTestSize
...
def parallelMap(numberOfCPUs, f, *xs):
    ...
    def safeCall(x):
        try:
            ...
            print(
                "Exception in worker during parallel map:\n%s"
