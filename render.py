...
def render(
    ...
    if len(sources) > 100:
        prefix = render(
            sources[:100], showImage, yieldsPixels, canvas, x0y0, resolution
        )
        suffix = render(
            sources[100:], showImage, yieldsPixels, canvas, x0y0, resolution
        )
...
""" % (
    ...
    preamble += "\n"
    sources = [preamble + canvas + "\n" + s + "\n\\end{tikzpicture}" for s in sources]
    source = "\n\n\n".join(sources)
    source = """
...
""" % (
    ...
    os.system(
        "cd /tmp; echo X|pdflatex -shell-escape %s 2> /dev/null > /dev/null"
        % temporaryName
    )
    ...
    if len(sources) > 1:
        ...
        if not all([os.path.isfile(t) for t in temporaryImages]):
            print(
                "WARNING: didn't render to zero prefixed filenames. Trying without prefixes:"
            )
            # for sketch2
            pattern = "%s-%d.png"
            temporaryImages = [
                pattern % (temporaryPrefix, j) for j in range(len(sources))
            ]
            if not all([os.path.isfile(t) for t in temporaryImages]):
                print(
                    "ERROR: didn't get any files without zero prefixes either. Here is the latex output:"
                )

                raise Exception(
                    "No image output from latex process. prefix = %s, len(sources) = %d, fs = \n%s\n,checks = \n%s\n"
                    % (
                        temporaryPrefix,
                        len(sources),
                        "\n".join(temporaryImages),
                        "\n".join(
                            map(str, [os.path.isfile(t) for t in temporaryImages])
                        ),
                    )
                )
            else:
...
def animateMatrices(matrices, outputFilename=None):
    ...
    ani = animation.FuncAnimation(
        fig, updatefig, frames=list(range(len(matrices))), interval=50, blit=True
    )
