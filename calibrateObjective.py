...
def featuresOfParticle(p, target):
    f = list(
        map(
            float, [p.logLikelihood, p.program.logPrior(), p.distance[0], p.distance[1]]
        )
    )
    return f
        ...
        for t, positives, negatives in distanceTrainingData:
            positiveScores = [
                asymmetricBlurredDistance(
                    t, p, kernelSize=kernelSize, factor=factor, invariance=invariance
                )
                for p in positives
            ]
            negativeScores = [
