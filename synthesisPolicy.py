...
class SynthesisPolicy:  # nn.Module):
    ...
    def scoreJobs(self, jobs):
        ...
        for j in jobs:
            score = (
                binary(y[0], j.incremental)
                + binary(y[1], j.canLoop)
                + binary(y[2], j.canReflect)
            )
    ...
    def expectedTime(self, results):
        ...
        TIMEOUT = 999
        minimumCost = min(
            [results[j].cost for j in jobs if results[j].cost != None] + [TIMEOUT]
        )
        if minimumCost == TIMEOUT:
            print("TIMEOUT", sequence)
            assert False
        successes = [
            results[j].cost != None and results[j].cost <= minimumCost + 1 for j in jobs
        ]
    ...
    def deepCoderLoss(self, results):
        ...
        bestResult = min(
            list(results.values()), key=lambda r: r.cost if r.cost != None else TIMEOUT
        )
        ...
        for k in p.walk():
            if isinstance(k, Loop):
                loops = True
            elif isinstance(k, Reflection):
                reflects = True
            if loops and reflects:
                break
        ...
        return -(
            binary(y[0], incremental) + binary(y[1], loops) + binary(y[2], reflects)
        )  # + y[2 + depth] - z)
    ...
    def learn(
        ...
        for s in range(1, numberOfIterations + 1):
            if L == "expected":
                ...
                testingLoss = (
                    sum([self.expectedTime(results) for results in testingData]).data[0]
                    if testingData != []
                    else 0.0
                )
            elif L == "bias":
                ...
                testingLoss = (
                    sum(
                        [self.biasOptimalTime(results, B) for results in testingData]
                    ).data[0]
                    if testingData != []
                    else 0.0
                )
                ...
                testingLoss = (
                    sum(self.deepCoderLoss(results) for results in data).data[0]
                    if testingData != []
                    else 0.0
                )
            else:
            ...
            if testingData != []:
                testingLoss = testingLoss * len(data) / len(testingData)
            if s % 10 == 0:
                print(
                    "%d/%d : training loss = %.2f : testing loss = %.2f : ETA this fold = %.2f hours : ETA all folds = %.2f hours"
                    % (s, numberOfIterations, loss.data[0], testingLoss, ETAthis, ETA)
                )
        ...
        for s in range(100):
            L = sum(
                [
                    R * ll
                    for results in data
                    for (R, ll) in [self.rollout(results, True)]
                ]
            )
            L = L / len(data)
            print(L)
    ...
    def featureExtractor(sequence):
        ...
        basicFeatures = [
            len([x for x in sequence.lines if isinstance(x, k)])
    ...
    def rollout(self, results, returnLogLikelihood=False, L="expected"):
        ...
        TIMEOUT = 999
        minimumCost = min(
            [r.cost for r in list(results.values()) if r.cost != None] + [TIMEOUT]
        )
        if minimumCost == TIMEOUT:
            print("TIMEOUT", sequence)
            assert False
            ...
            while True:
                candidates = [
                    ...
                    if not any(
                        [
                            finished == j
                            or (results[finished].cost != None and finished.subsumes(j))
                ...
                resourceDistribution = [
                    math.exp(jobLogLikelihood[j].data[0] - z) for j in candidates
                ]
                timeToFinishEachCandidate = [
                    (results[j].time - jobProgress[j]) / w
                    for w, j in zip(resourceDistribution, candidates)
                ]
                (dt, nextResult) = min(list(zip(timeToFinishEachCandidate, candidates)))
                T += dt
                if (
                    results[nextResult].cost != None
                    and results[nextResult].cost <= minimumCost + 1
                ):
                    return T
        ...
        if L == "DC":
            ...
            for (canLoop, canReflect) in attemptSequence:
                ...
                for d in [2, 3]:  # range(1,4):
                    ...
                    result = results[j1]
                    T += result.time
                    if result.cost != None and result.cost <= minimumCost + 1:
                        return T
                    j2 = [
                        j
                        for j in jobs
        ...
        while True:
            candidates = [
                ...
                if not any(
                    [
                        str(j) == str(o) or (results[o].cost != None and o.subsumes(j))
                        for o in history
                    ]
                )
            ]
            if candidates == []:
                ...
                assert False
            job = candidates[
                sampleLogMultinomial([jobLogLikelihood[j].data[0] for j in candidates])
            ]
            sample = results[job]
            time += sample.time
            history.append(job)
            ...
            if returnLogLikelihood:
                trajectoryLogProbability = (
                    jobLogLikelihood[job] + trajectoryLogProbability
                )
    ...
    def timeshare(
        ...
        tasks = [
            TimeshareTask(invokeExecuteMethod, [j], s, timeout=2 * 60 * 60)
            for j, s in zip(jobs, scores)
        ]
        bestResult = None
        resultIndex = 0
        for result in executeTimeshareTasksFairly(
            tasks,
            ...
            if result.cost != None:
                ...
                if bestResult == None or bestResult.cost > result.cost:
                    bestResult = result
                if result.cost <= optimalCost + 1 and globalTimeout is None:
                    break
                for t in tasks:
                    if result.job.subsumes(t.arguments[0]):
                        t.finished = True
        for t in tasks:
            t.cleanup()
    ...
    for j in range(100):
        ...
        resultsArray.append(
            dict(
                [
                    ...
                    and r.job.originalDrawing == drawing
                ]
            )
        )
        ...
        for job1, result1 in resultsArray[-1].items():
            for job2, result2 in resultsArray[-1].items():
                if job1.subsumes(
                    job2
                ):  # job1 is more general which implies that either there is no result or it is better than the result for job2
                    if not (
                        result1.cost == None
                        or result2.cost == None
                        or result1.cost <= result2.cost
                    ):
...
def analyzePossibleFeatures(data):
    ...
    for results in data:
        for j in results:
            ...
            if r.cost != None and r.program == None:
                ...
                try:
                    r.program = parseSketchOutput(results[j].source)
                except:
                    r.cost = None
    ...
    iterativeScores = [
        (flag, (len(x) + len(y)) / float(len(parse)))
...
def bestPossibleTime(results):
    ...
    return min(
        [
            ...
            if r.cost != None and r.cost <= minimumCost + 1
        ]
        + [TIMEOUT]
    )
...
def exactTime(results):
    ...
    return min(
        [
            ...
            and j.canLoop
            and j.canReflect
            and j.maximumDepth == 3
            and r.cost != None
            and r.cost <= minimumCost + 1
        ]
        + [TIMEOUT]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="training and evaluation of synthesis policies"
    )
    parser.add_argument(
        "-f",
        "--features",
    ...
    parser.add_argument(
        "-m", "--mode", choices=["expected", "bias", "DC+bias", "DC"], default="bias"
    )
    ...
    for mode in modes:
        ...
        for train, test in crossValidate(data, numberOfFolds, randomSeed=42):
            path = "checkpoints/policy_%s_%s_%s%d_%d.pt" % (
                arguments.features,
                mode,
                ""
                if arguments.regularize == 0
                else "regularize%f_" % arguments.regularize,
                foldCounter,
                arguments.folds,
            )
            foldCounter += 1
            ...
            if arguments.evaluate is None:
                policy += [
                    model.rollout(r, L=mode)
                    for r in test
                    for _ in range(10 if mode == "expected" else 1)
                ]
            else:
    ...
    if arguments.evaluate is not None:
        ...
        def policyEvaluator(problemIndex):
            try:
                ...
                with open(problemIndex, "rb") as handle:
                    particle = pickle.load(handle)
                parse = particle.sequence()
                try:
                    problemIndex = int(
                        re.search("expert-(\d+)-p", problemIndex).group(1)
                    )
                except:
                    problemIndex = None
            ...
            if problemIndex is not None:
                costs = [
                    r.cost for _, r in data[problemIndex].items() if r.cost != None
                ]
                if costs == []:
                    return None
                bestCost = min(costs)
                ...
                job2w = dict(
                    list(
                        zip(
                            ...
                            np.exp(
                                normalizeLogs(
                                    np.array([s.data[0] for s in model.scoreJobs(jobs)])
                                )
                            ),
                        )
                    )
            ...
            startTime = time.time()
            result = model.timeshare(
                problemIndex,
                bestCost,
                globalTimeout=arguments.timeout,
                verbose=True,
                parse=parse,
    ...
    if "DC" in policyRollouts:
        modelsToCompare.append((policyRollouts["DC"], "DC"))
    modelsToCompare.append((optimistic, "oracle"))
    if "bias" in policyRollouts:
        modelsToCompare.append((policyRollouts["bias"], "learned policy (ours)"))
    ...
    for j, (ys, l) in enumerate(modelsToCompare):
        ...
        print(
            l,
            "timeouts or gives the wrong answer",
            len([y for y in ys if y == TIMEOUT]),
            "times",
        )
        median = np.median(ys)
    ...
    # plot.plot()
    figure.text(0.5, 0.04, "time (sec)", ha="center", va="center", fontsize=9)
    plot.tight_layout()

    figureFilename = "policyComparison_%s_%s_%d.png" % (
        arguments.features,
        arguments.mode,
        arguments.folds,
    )
