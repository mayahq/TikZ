...
class BatchIterator:
    def __init__(
        self, batchSize, tensors, testingFraction=0.0, stringProcessor=None, seed=42
    ):
        for t in tensors:
            assert t.shape[0] == tensors[0].shape[0]
    ...
    def __next__(self):
        ...
        batch = tuple(
            [
                self.processTensor(t[self.startingIndex : endingIndex, ...])
                for t in self.tensors
            ]
        )
        self.startingIndex = endingIndex
        if self.startingIndex == self.trainingSetSize:
            self.startingIndex = 0
        return batch
    ...
    def testingSlice(self, start, size):
        return [
            self.processTensor(t[start : (start + size), ...])
            for t in self.testingTensors
        ]
        ...
        while True:
            yield dict(
                list(
                    zip(
                        self.placeholders,
                        self.testingSlice(testingIndex, self.batchSize),
                    )
                )
            )
            testingIndex += self.batchSize
            if testingIndex >= self.testingSetSize:
                break
