...
class AbstractionVariable:
    ...
    def __eq__(self, o):
        return isinstance(o, AbstractionVariable) and self.v == o.v

    def __str__(self):
        return "__v__(%d)" % (self.v)
...
class Environment:
    ...
    def extend(self, x, y=None):
        if y == None:
            x, y = AbstractionVariable(len(self.bindings)), x
        return Environment(self.bindings + [(x, y)]), x

    def __str__(self):
        return "Environment([%s])" % (
            ", ".join(["%s -> %s" % (k, v) for k, v in self.bindings])
        )
    ...
    def randomInstantiation(self):
        ...
        for v, (x, y) in self.bindings:
            if isinstance(x, int):
                ...
                average = (x + y) / 2.0
                standardDeviation = (
                    ((x - average) ** 2 + (y - average) ** 2) / 2.0
                ) ** (0.5)
                z = int(np.random.normal(loc=average, scale=standardDeviation))
...
class RelativeExpression:
    orientations = {"n": "North", "w": "West", "e": "East", "s": "South"}
    ...
    def __str__(self):
        return "%s(%d)" % (
            RelativeExpression.orientations[self.orientation],
            self.index,
        )
    ...
    def __eq__(self, o):
        return (
            isinstance(o, RelativeExpression)
            and o.index == self.index
            and o.orientation == self.orientation
        )
...
class Primitive:
    def pretty(self):
        ...
        p = "%s(%s)" % (
            self.k,
            ",".join([(a if isinstance(a, str) else a.pretty()) for a in arguments]),
        )
    ...
    def __str__(self):
        return "Primitive(%s)" % (", ".join(map(str, [self.k] + self.arguments)))
    ...
    def evaluate(self, e):
        return set([self.evaluate_(e)])

    def evaluate_(self, e):
        if self.k == "circle":
            return Circle.absolute(
                self.arguments[0].evaluate(e), self.arguments[1].evaluate(e)
            )
        if self.k == "rectangle":
            return Rectangle.absolute(
                self.arguments[0].evaluate(e),
    ...
    def fixStringParameters(self):
        if self.k == "line":
            ...
            solid = self.arguments[5]
            if isinstance(arrow, str):
                arguments[4] = "True" in arrow
            if isinstance(solid, str):
                arguments[5] = "True" in solid
    ...
    def canonicalKey(self):
        return tuple(
            [self.k]
            + [
                a if isinstance(a, (str, bool)) else a.canonicalKey()
    ...
    def mapExpression(self, l):
        return Primitive(
            self.k,
            *[(l(a) if isinstance(a, LinearExpression) else a) for a in self.arguments],
        )

    @staticmethod
    def enumerate(environment):
        if any([isinstance(x, Circle) for x in environment.difference]):
            for x in LinearExpression.enumerate(
                list(environment.variableRanges.keys())
            ):
                for y in LinearExpression.enumerate(
                    list(environment.variableRanges.keys())
                ):
    ...
    def substitute(self, e):
        return Primitive(
            self.k,
            *[
                (a.substitute(e) if isinstance(a, LinearExpression) else a)
                for a in self.arguments
            ],
        )
...
class Reflection:
    ...
    def fixStringParameters(self):
        return Reflection(self.axis, self.coordinate, self.body.fixStringParameters())
    ...
    def abstract(self, other, e):
        if not isinstance(other, Reflection):
            raise AbstractionFailure("abstracting a reflection with a not reflection")
        if self.axis == other.axis:
            axis = self.axis
        else:
            axis, e = e.makeVariableWithValue((self.axis, other.axis))
        if self.coordinate == other.coordinate:
            coordinate = self.coordinate
        else:
            coordinate, e = e.makeVariableWithValue((self.coordinate, other.coordinate))
...
class Loop:
    ...
    def fixStringParameters(self):
        return Loop(
            ...
            else self.boundary.fixStringParameters(),
    ...
    def removeDeadCode(self):
        ...
        if boundary != None and boundary.items == []:
            boundary = None
        return Loop(
            self.v, self.bound, body, boundary=boundary, lowerBound=self.lowerBound
        )
    ...
    def __str__(self):
        if self.boundary != None:
            return "Loop(%s, %s, %s, %s, boundary = %s)" % (
                self.v,
                self.lowerBound,
                self.bound,
                self.body,
    ...
    def extrapolations(self):
        for b in self.body.extrapolations():
            for boundary in (
                [None] if self.boundary == None else self.boundary.extrapolations()
            ):
                for ub, lb in [(1, 0), (0, 1), (1, 1), (0, 0)]:
                    yield Loop(
                        self.v,
                        self.bound.offset(ub),
                        b,
    ...
    def explode(self):
        shrapnel = [
            Loop(
                self.v, self.bound, bodyExpression.explode(), lowerBound=self.lowerBound
            )
    ...
    def mergeWithOtherLoop(self, other):
        ...
        if other.boundary != None:
            if boundary == None:
                ...
                boundary = Block(self.boundary.items + other.boundary.items)
    ...
    def mergeWithOtherLoopDifferentBounds(self, other):
        ...
        boundary = other.body.mapExpression(
            lambda l: LinearExpression(l.m, l.x, l.b - l.m) if l.x == self.v else l
        ).items
        if self.boundary != None:
            boundary = boundary + self.boundary.items
    ...
    def walk(self):
        yield self
        for x in self.body.walk():
            yield x
        if self.boundary != None:
            for x in self.boundary.walk():
                yield x
    ...
    def cost(self):
        ...
        if self.bound.m == 0 and self.bound.b == 2:
            cost += 1
        return cost + 1

    def abstract(self, other, e):
        if (
            not isinstance(other, Loop)
            or self.v != other.v
            or ((other.boundary == None) != (self.boundary == None))
        ):
        ...
        if self.boundary == None:
            ...
            boundary, e = self.boundary.abstract(other.boundary, e)
    ...
    def depth(self):
        return 1 + max(
            self.body.depth(), 0 if self.boundary == None else self.boundary.depth()
        )
    ...
    def enumerate(environment):
        ...
        if len(environment.variableRanges) < 2:
            ...
            for bound in LinearExpression.enumerate(
                list(environment.variableRanges.keys()),
                maximumIntercept=4,
                maximumSlope=2,
            ):
                if bound.m == 0 and bound.b < 2:
                    continue
...
class Block:
    ...
    def __str__(self):
        return "Block([%s])" % (", ".join(map(str, self.items)))
    ...
    def evaluate(self, environment):
        accumulator = set([])
        for x in self.items:
            accumulator |= x.evaluate(environment)
        return accumulator

    def canonicalKey(self):
        return tuple([x.canonicalKey() for x in self.items])

    def canonical(self):
        return Block(
            list(
                sorted(
                    (x.canonical() for x in self.items), key=lambda y: y.canonicalKey()
                )
            )
        )
    ...
    def removeDeadCode(self):
        ...
        keptItems = []
        for x in items:
            if isinstance(x, Reflection) and x.body.items != []:
                keptItems.append(x)
            elif isinstance(x, Loop) and (
                x.body.items != [] or (x.boundary != None and x.boundary.items != [])
            ):
                keptItems.append(x)
    ...
    def rewriteUpToDepth(self, d):
        rewrites = [[self]]
        for _ in range(d):
            rewrites.append([r for b in rewrites[-1] for r in b.rewrites()])
        ...
        for j, x in enumerate(self.items):
            for k, y in enumerate(self.items):
                ...
                if isinstance(x, Loop) and isinstance(y, Loop):
                    ...
                    if (
                        x.bound.m == y.bound.m
                        and x.bound.x == y.bound.x
                        and abs(x.bound.b - y.bound.b) == 1
                    ):
                        # make it so that y is the one with fewer iterations
                        if x.bound.b < y.bound.b:
                            x, y = y, x
                        if y.boundary != None:
                            continue
                ...
                if isinstance(x, Reflection) and isinstance(y, Reflection):
                    if (x.axis, x.coordinate) == (y.axis, y.coordinate):
                        newItems = list(self.items)
                        newItems[j] = Reflection(
                            x.axis, x.coordinate, Block(x.body.items + y.body.items)
                        )
                        del newItems[k]
                        yield Block(newItems)
    ...
    def totalCost(self):
        ...
        boundaryCount = 0
        for x in self.walk():
            if isinstance(x, Loop) and x.boundary != None:
                boundaryCount += 1
        boundaryCount = 0
...
def parseSketchOutput(output, environment=None, loopDepth=0, coefficients=None):
    ...
    while j < len(output):
        l = output[j]
        if "void renderSpecification" in l:
            break
        ...
        if m and (not ("reflectionIndex" in m.group(1))):
            ...
            if boundaryIndex != None:
                ...
                boundary = parseSketchOutput(
                    boundary, environment, loopDepth + 1, coefficients
                )
                j = boundaryIndex
            else:
                boundary = None
...
def parseExpression(e):
    ...
    try:
        return LinearExpression(0, None, int(e))
    except:
        factor = re.search("([\-0-9]+) \* ", e)
        if factor != None:
            factor = int(factor.group(1))
        ...
        if variable != None:
            variable = ["i", "j"][int(variable.group(1))]
...
if __name__ == "__main__":
    ...
    start = Block(
        [
            Loop(
                "j",
                LinearExpression(0, None, 3),
                Block([Primitive("x")]),
                boundary=Block([Primitive("w", LinearExpression(5, "j", -1))]),
            ),
            Loop(
                "j",
                LinearExpression(0, None, 2),
