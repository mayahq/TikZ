...
class RecurrentNetwork:
    def __init__(
        ...
        maximumLength,
        inputFeatures=None,
        alwaysProvideInput=False,
    ):
        self.model = rnn.LSTMCell(numberOfUnits)

        self.loadingMatrix = tf.Variable(
            tf.random_uniform([numberOfUnits, dictionarySize], -1.0, 1.0),
            name="LOADINGMATRIX",
        )
        ...
        if inputFeatures != None:
            self.transformedInputFeatures = [
                tf.layers.dense(inputs=inputFeatures, units=s, activation=tf.nn.tanh)
                for s in self.model.state_size
            ]
            self.transformedInputFeatures = rnn.LSTMStateTuple(
                *self.transformedInputFeatures
            )
            if alwaysProvideInput:
                self.alwaysProvidedInput = tf.layers.dense(
                    inputs=inputFeatures, units=numberOfUnits, activation=tf.nn.tanh
                )
            else:
        ...
        self.inputPlaceholder = tf.placeholder(
            tf.int32, shape=[None, maximumLength], name="INPUT"
        )
        embeddedInputs = tf.nn.embedding_lookup(
            tf.transpose(self.loadingMatrix), self.inputPlaceholder
        )
        if alwaysProvideInput:
            ...
            alwaysProvidedInput2 = tf.reshape(
                self.alwaysProvidedInput, [-1, 1, numberOfUnits]
            )
        ...
        projectedOutputs = tf.tensordot(
            self.outputs, self.loadingMatrix, axes=[[2], [0]]
        )
        ...
        self.statePlaceholders = [
            tf.placeholder(tf.float32, [None, numberOfUnits], name="state0"),
            tf.placeholder(tf.float32, [None, numberOfUnits], name="state1"),
        ]
        self.oneInputPlaceholder = tf.placeholder(
            tf.int32, shape=[None], name="inputForOneStep"
        )
        projectedInputs = tf.nn.embedding_lookup(
            tf.transpose(self.loadingMatrix), self.oneInputPlaceholder
        )
        if alwaysProvideInput:
            projectedInputs = projectedInputs + self.alwaysProvidedInput
        self.oneOutput, self.oneNewState = self.model(
            projectedInputs, rnn.LSTMStateTuple(*self.statePlaceholders)
        )
        self.oneNewState = [self.oneNewState[0], self.oneNewState[1]]
        self.oneOutputDistribution = tf.nn.log_softmax(
            tf.matmul(self.oneOutput, self.loadingMatrix)
        )
    ...
    def decodesIntoLoss(self, labels):
        l = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=labels, logits=self.outputDistribution
        )
        # l:labels
        # l:None,L
    ...
    def decodesIntoAccuracy(self, labels, perSymbol=True):
        ...
        if perSymbol:
            ...
            accuracyVector = tf.divide(
                tf.cast(accuracyVector, tf.float32),
                tf.cast(self.lengthPlaceholder, tf.float32),
            )
        if not perSymbol:
            # accuracy is measured per sequence
            accuracyVector = tf.cast(
                tf.equal(accuracyVector, self.lengthPlaceholder), tf.float32
            )
    ...
    def sample(self, session, stopSymbol=None, baseFeed=None):
        ...
        for j in range(self.maximumLength):
            ...
            [s0, s1, outputDistribution] = session.run(
                self.oneNewState + [self.oneOutputDistribution], feed
            )
    ...
    def beam(
        ...
        if stopSymbol != None:
            sequenceChecker = (
                lambda sq: RecurrentNetwork.FINISHEDSEQUENCE
                if sq[-1] == stopSymbol
                else RecurrentNetwork.VALIDSEQUENCE
            )
        ...
        for j in range(maximumLength):
            ...
            feed[self.oneInputPlaceholder] = np.array(
                [([0] + s)[-1] for _, s, _ in particles]
            )
            feed[self.statePlaceholders[0]] = np.array(
                [s for _, _, [s, _] in particles]
            )
            feed[self.statePlaceholders[1]] = np.array(
                [s for _, _, [_, s] in particles]
            )
            if self.alwaysProvidedInput != None:
                feed[self.alwaysProvidedInput] = np.array([constantInput] * B)

            [newStates0, newStates1, distribution] = session.run(
                self.oneNewState + [self.oneOutputDistribution], feed
            )

            particles = [
                (
                    particles[b][0] + distribution[b, w],
                    particles[b][1] + [w],
            ...
            if sequenceChecker != None:
                ...
                finishedParticles += [
                    p
                    for check, p in zip(checks, particles)
...
if __name__ == "__main__":
    ...
    with tf.Session() as session:
        ...
        for i in range(10000):
            ...
            l, a, _ = session.run([loss, accuracy, Optimizer], feed)
            if i % 1000 == 0:
                print(i, l, a)
        for h in [1.0, 2.0]:
            print("Sampling h = %s" % h)
            samples = [
                tuple(m.sample(session, stopSymbol=0, baseFeed={hint: np.array([[h]])}))
                for _ in range(1000)
            ]
            histogram = {}
            for s in samples:
