...
class StandardPrimitiveDecoder:
    def makeNetwork(self, imageRepresentation):
        ...
        for j, (t, d) in enumerate(self.outputDimensions):
            ...
            if j in self.attentionIndices:
                theta0 = (
                    np.array([[1.0, 0, 0], [0, 1.0, 0]]).astype("float32").flatten()
                )
                theta = tf.layers.dense(
                    tf.concat(predictionInputs[1:], axis=1),
                    6,
                ...
                transformed = spatial_transformer_network(
                    imageRepresentation, theta, (self.attentionSize, self.attentionSize)
                )
                flat = tf.reshape(
                    transformed, [-1, self.attentionSize * self.attentionSize * C]
                )
            ...
            if t == int:
                ...
                self.loss.append(
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        labels=self.targetPlaceholder[j], logits=p
                    )
                )
            elif t == float:
                mixtureParameters = mixtureDensityLayer(
                    d,
                    intermediateRepresentation,
                    epsilon=0.01,
                ...
                self.loss.append(
                    -mixtureDensityLogLikelihood(
                        mixtureParameters, self.targetPlaceholder[j]
                    )
                )
                self.soft += [None]
                self.hard += [None]
    ...
    def accuracyVector(self):
        ...
        hard = [
            tf.equal(h, t)
            for h, t in zip(self.hard, self.targetPlaceholder)
            if h != None
        ]
        if hard != []:
    ...
    def beamTrace(self, session, feed, beamSize):
        ...
        for j in range(len(self.outputDimensions)):
            ...
            if self.outputDimensions[j][0] == int:
                ...
                traces = [
                    (s + coordinateScore, trace + [coordinateIndex])
                ...
                traces = [
                    (s + coordinateScore, trace + [coordinate])
                    for traceIndex, (s, trace) in enumerate(traces)
                    for coordinate, coordinateScore in beamMixture(
                        u[traceIndex],
                        v[traceIndex],
                        p[traceIndex],
                        1,
    ...
    def attentionSequence(self, session, feed, l):
        ...
        # makes a copy of the feed
        feed = dict([(k, feed[k]) for k in feed])
        for t, p in zip(ts, self.targetPlaceholder):
            feed[p] = np.array([t])
        return session.run(self.attentionTransforms, feed_dict=feed)
...
class CircleDecoder(StandardPrimitiveDecoder):
    ...
    def beam(self, session, feed, beamSize):
        if NIPSPRIMITIVES():
            r = 1
            return [
                (s, Circle(AbsolutePoint(x, y), r))
                for s, [x, y] in self.beamTrace(session, feed, beamSize)
                if x - r > 0
                and y - r > 0
                and x + r < MAXIMUMCOORDINATE
                and y + r < MAXIMUMCOORDINATE
            ]
        else:
            return [
                (s, Circle(AbsolutePoint(x, y), r))
    ...
    def extractTargets(l):
        if l != None and isinstance(l, Circle):
            return [l.center.x, l.center.y] + ([] if NIPSPRIMITIVES() else [l.radius])
...
class LabelDecoder(StandardPrimitiveDecoder):
    ...
    def __init__(self, imageRepresentation, continuous, attention):
        if attention > 0:
            ...
            self.attentionSize = attention
        if continuous:
            self.outputDimensions = [(float, MAXIMUMCOORDINATE)] * 2 + [
                (int, len(Label.allowedLabels))
            ]  # x,y,c
            self.hiddenSizes = [None, None, None]
        else:
            self.outputDimensions = [(int, MAXIMUMCOORDINATE)] * 2 + [
                (int, len(Label.allowedLabels))
            ]  # x,y,c
            self.hiddenSizes = [None, None, None]
        self.makeNetwork(imageRepresentation)
    ...
    def beam(self, session, feed, beamSize):
        return [
            (s, Label(AbsolutePoint(x, y), Label.allowedLabels[l]))
    ...
    def extractTargets(l):
        if l != None and isinstance(l, Label):
            return [l.p.x, l.p.y, Label.allowedLabels.index(l.c)]
        return [0, 0, 0]
...
class RectangleDecoder(StandardPrimitiveDecoder):
    ...
    def beam(self, session, feed, beamSize):
        return [
            (s, Rectangle.absolute(x1, y1, x2, y2))
...
class LineDecoder(StandardPrimitiveDecoder):
    ...
    def __init__(self, imageRepresentation, continuous, attention):
        if attention > 0:
            ...
            self.attentionSize = attention
        if continuous:
            self.outputDimensions = [(float, MAXIMUMCOORDINATE)] * 4 + [
                (int, 2)
            ] * 2  # x,y for beginning and end; arrow/-
        else:
            self.outputDimensions = [(int, MAXIMUMCOORDINATE)] * 4 + [
                (int, 2)
            ] * 2  # x,y for beginning and end; arrow/-
        self.hiddenSizes = [None, 32, 32, 32, None, None]
        self.makeNetwork(imageRepresentation)

    def beam(self, session, feed, beamSize):
        return [
            (s, Line.absolute(x1, y1, x2, y2, arrow=arrow == 1, solid=solid == 1))
            for s, [x1, y1, x2, y2, arrow, solid] in self.beamTrace(
                session, feed, beamSize
            )
...
class StopDecoder:
    ...
    def softPredictions(self):
        return []

    @staticmethod
    def extractTargets(_):
        return []
...
class PrimitiveDecoder:
    ...
    decoderClasses = (
        [CircleDecoder, RectangleDecoder, LineDecoder, StopDecoder]
        if NIPSPRIMITIVES()
        else [CircleDecoder, RectangleDecoder, LineDecoder, LabelDecoder, StopDecoder]
    )

    def __init__(
        self, imageRepresentation, trainingPredicatePlaceholder, continuous, attention
    ):
        self.decoders = [
            k(imageRepresentation, continuous, attention)
            for k in PrimitiveDecoder.decoderClasses
        ]

        self.imageRepresentation = imageRepresentation
        self.prediction = tf.layers.dense(
            flattenImageOutput(self.imageRepresentation), len(self.decoders)
        )
    ...
    def loss(self):
        ...
        ll = tf.reduce_sum(
            tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.targetPlaceholder, logits=self.prediction
            )
        )
        for decoder in self.decoders:
            decoderMask = tf.cast(
                tf.equal(self.targetPlaceholder, decoder.token), tf.float32
            )
    ...
    def accuracy(self):
        ...
        for decoder in self.decoders:
            if decoder.token != STOP:
                ...
                if vector != True:
                    a = tf.logical_and(
                        a, tf.logical_or(vector, tf.not_equal(self.hard, decoder.token))
                    )
    ...
    def beam(self, session, feed, beamSize, maximumLength=None):
        ...
        [tokenScores, imageRepresentation] = session.run(
            [self.soft, self.imageRepresentation], feed_dict=feed
        )
...
class RecurrentDecoder:
    def __init__(
        self, imageFeatures, trainingPredicatePlaceholder, continuous, attention
    ):
        ...
        MAXIMUMRECURRENT = 1 + MAXIMUMPRIMITIVES * (
            7
        )  #  + 1stop symbol, MAXIMUMPRIMITIVES instructions, 6+1 arguments for a line

        self.trainingPredicatePlaceholder = trainingPredicatePlaceholder
        self.outputPlaceholder = tf.placeholder(
            tf.int32, shape=[None, MAXIMUMRECURRENT], name="recurrentOutputPlaceholder"
        )

        self.imageRepresentation = flattenImageOutput(imageFeatures)
        self.unit = RecurrentNetwork(
            arguments.LSTM,
    ...
    def beam(self, session, feed, k, maximumLength=None):
        feed = {
            self.imageRepresentation: session.run(self.imageRepresentation, feed)[0]
        }

        primitiveArguments = [
            [MAXIMUMCOORDINATE, MAXIMUMCOORDINATE],  # circle
            [MAXIMUMCOORDINATE] * 4,  # rectangle
            [MAXIMUMCOORDINATE] * 4 + [2, 2],  # line
            [],
        ]  # stop
        builders = [
            lambda x, y: Circle(AbsolutePoint(x, y), 1),
            lambda a, b, p, q: Rectangle.absolute(a, b, p, q),
            lambda a, b, p, q, arrow, solid: Line.absolute(
                a, b, p, q, arrow=arrow == 1, solid=solid == 1
            ),
        ]
...
class RecognitionModel:
    def __init__(self, arguments):
        ...
        with self.session.graph.as_default():
            ...
            if self.arguments.LSTM:
                ...
                imageInput = tf.stack(
                    [self.currentPlaceholder, self.goalPlaceholder], axis=3
                )
            ...
            self.decoder = decoderClass(
                c1,
                self.trainingPredicatePlaceholder,
    ...
    def train(self, numberOfExamples, restore=False):
        noisyTarget, programs = loadExamples(
            numberOfExamples, self.arguments.trainingData
        )

        iterator = BatchIterator(
            10,
            (np.array(noisyTarget), np.array(programs)),
            testingFraction=TESTINGFRACTION,
            stringProcessor=loadImage,
        )
        flushEverything()
        ...
        with self.session.graph.as_default():
            ...
            for e in range(100):
                ...
                for ts, ps in iterator.epochExamples():
                    ...
                    _, l, accuracy = self.session.run(
                        [self.optimizer, self.loss, self.averageAccuracy],
                        feed_dict=feed,
                    )
                ...
                print(
                    "Epoch %d: accuracy = %f, loss = %f"
                    % (
                        (e + 1),
                        sum(epicAccuracy) / len(epicAccuracy),
                        sum(epicLoss) / len(epicLoss),
                    )
                )
                testingAccuracy = []
                for ts, ps in iterator.testingExamples():
                    feed = self.makeTrainingFeed(ts, ps)
                    feed[self.trainingPredicatePlaceholder] = False
                    testingAccuracy.append(
                        self.session.run(self.averageAccuracy, feed_dict=feed)
                    )
                print(
                    "\tTesting accuracy = %f"
                    % (sum(testingAccuracy) / len(testingAccuracy))
                )
                print(
                    "Saving checkpoint: %s"
                    % saver.save(self.session, self.checkpointPath)
                )
                flushEverything()
    ...
    def makeTrainingFeed(self, targets, programs):
        ...
        if self.arguments.LSTM:
            feed = self.decoder.unit.decodingTrainingFeed(
                ps, self.decoder.outputPlaceholder
            )
        ...
        f = {self.goalPlaceholder: gs, self.currentPlaceholder: cs}
    ...
    def beam(self, current, goal, beamSize, maximumLength=None):
        if self.arguments.LSTM:
            ...
            feed = {
                self.currentPlaceholder: np.array([current]),
                self.goalPlaceholder: np.array([goal]),
            }
        return sorted(
            self.decoder.beam(
                self.session, feed, beamSize, maximumLength=maximumLength
            ),
            reverse=True,
        )
    ...
    def analyzeFailures(self, numberOfExamples):
        ...
        with self.session.graph.as_default():
            ...
            for ts, ps in iterator.testingExamples():
                ...
                for j in range(len(targetProgram)):
                    ...
                    singleFeed = dict(
                        [
                            (placeholder, np.array([feed[placeholder][j, ...]]))
                            for placeholder in feed
                        ]
                    )
                    current, goal = (
                        singleFeed[self.currentPlaceholder][0],
                        singleFeed[self.goalPlaceholder][0],
                    )
                    target = targetProgram.lines[j]
                    if self.arguments.continuous:
                        target = target.round(CONTINUOUSROUNDING)
                    ...
                    if predictions[0][1] != target:
                        ...
                        if self.arguments.attention > 0:
                            ...
                            if attention != []:
                                print(attention)
                                print(target)
                                illustration = drawAttentionSequence(
                                    goal, attention, target
                                )
                                saveMatrixAsImage(
                                    illustration,
                                    "attentionIllustrations/%d.png"
                                    % (totalNumberOfAttempts - len(failures)),
                                )
        ...
        ranks = [
            None
            if not f["target"] in list(map(snd, f["predictions"]))
            else list(map(snd, f["predictions"])).index(f["target"]) + 1
            for f in failures
        ]
        print(ranks)
        print(
            "In the frontier %d/%d" % (len([r for r in ranks if r != None]), len(ranks))
        )
        ...
        print(
            "Circle failures: %d"
            % (len([None for f in failures if isinstance(f["target"], Circle)]))
        )
        print(
            "Line failures: %d"
            % (len([None for f in failures if isinstance(f["target"], Line)]))
        )
        print(
            "Rectangle failures: %d"
            % (len([None for f in failures if isinstance(f["target"], Rectangle)]))
        )
        print(
            "Label failures: %d"
            % (len([None for f in failures if isinstance(f["target"], Label)]))
        )
        print(
            "Stop failures: %d" % (len([None for f in failures if None == f["target"]]))
        )
        ...
        for j, failure in enumerate(failures):
            ...
            p = Sequence(p).draw()
            saveMatrixAsImage(
                255 * (p + failure["current"]), "failures/%d-predicted.png" % j
            )
...
class Particle:
    ...
    def finished(self):
        return isinstance(self.program, Sequence)
...
class DistanceModel:
    def __init__(self, arguments):
        ...
        with self.session.graph.as_default():
            ...
            self.valueTargets = tf.placeholder(
                tf.float32, [None, 2]
            )  # (extra target, extra current)
            ...
            self.distanceLoss = tf.reduce_mean(
                tf.squared_difference(self.valueTargets, self.distanceFunction)
            )
            self.distanceOptimizer = tf.train.AdamOptimizer(
                learning_rate=self.arguments.learningRate
            ).minimize(self.distanceLoss)

    @property
    def checkpointPath(self):
        return "checkpoints/distance_%s_%s.checkpoint" % (
            self.arguments.architecture,
            "noisy" if self.arguments.noisy else "clean",
        )
    ...
    def learnedDistances(self, currentBatch, goalBatch):
        return self.session.run(
            ...
            feed_dict={
                self.currentPlaceholder: currentBatch,
    ...
    def learnedParticleDistances(self, goal, particles):
        ...
        d = self.learnedDistances(
            np.array([p.render() for p in particles]),
    ...
    def train(self, numberOfExamples, restore=False):
        ...
        targetImages, targetPrograms = loadExamples(
            numberOfExamples, self.arguments.trainingData
        )
        iterator = BatchIterator(
            5,
            tuple([np.array(targetImages), np.array(targetPrograms)]),
            testingFraction=TESTINGFRACTION,
            stringProcessor=loadImage,
        )
        ...
        with self.session.graph.as_default():
            ...
            for e in range(20):
                ...
                for images, programs in iterator.epochExamples():
                    targets, current, distances = makeDistanceExamples(
                        images,
                        programs,
                    ...
                    _, l = self.session.run(
                        [self.distanceOptimizer, self.distanceLoss],
                        feed_dict={
                            self.currentPlaceholder: current,
                    ...
                    if time() - lastUpdateTime > 120:
                        ...
                        print(
                            ...
                            % (runningAverage / runningAverageCount)
                        )
                        flushEverything()
                ...
                testingLosses = [
                    ...
                    for [targets, current, distances] in [
                        makeDistanceExamples(images, programs)
                    ]
                ]
            ...
            for m in range(3):
                ...
                if m > 0:
                    for _ in range(choice([1, 2, 3, 4])):
                        sp = sp.mutate()
                d = self.learnedDistances(
                    np.array([sp.draw()]), np.array([targetImages[j]])
                )[0]
...
class SearchModel:
    ...
    def sample(self, targetImage, maximumLength):
        ...
        for j in range(maximumLength):
            ...
            if nextCommand == None or j == maximumLength - 1:
                return Sequence(currentProgram)
    ...
    def SMC(self, targetImage, beamSize=10, beamLength=10):
        ...
        for iteration in range(beamLength if beamLength > 0 else 50):
            ...
            for parent in beam:
                ...
                if not self.arguments.unguided:
                    ...
                    kids = [
                        ...
                        for randomCode in [randomLineOfCode()]
                        if randomCode != None
                    ]
                    kids += [(0.0, None)]
                ...
                if (
                    self.arguments.task == "evaluate"
                    and (not [k for k in kids[:childCount] if k[1] == None])
                    and childCount > 1
                ):
            ...
            if lastIteration and self.arguments.task != "evaluate":
                children = [p for p in children if p.finished()]
            ...
            if self.arguments.beam:
                beam = sorted(beam, key=lambda p: p.logLikelihood, reverse=True)[
                    :beamSize
                ]
            ...
            if self.arguments.distance:  # use the learned distance metric
                ...
                for p in beam:
                    p.distance = (
                        p.distance[0] + self.arguments.mistakePenalty * p.distance[1]
                    )
            else:
                for n in beam:
                    if not self.arguments.noisy:
                        ...
                        n.distance = np.sum(
                            np.abs(difference[difference > 0])
                        ) + 20 * np.sum(np.abs(difference[difference < 0]))
                    else:
            ...
            for n in beam:
                ...
                if n.parent.count > 0:
                    n.score += math.log(
                        n.parent.count
                    )  # simulate affect of drawing repeatedly from previous distribution
                else:
                    assert (
                        self.arguments.beam
                    )  # should only occur in straight up beam search
                n.score += self.arguments.proposalCoefficient * (
                    n.logLikelihood - n.parent.logLikelihood
                )
                ...
                if not n.finished():
                    n.score += self.arguments.priorCoefficient * (
                        n.program[-1].logPrior()
                    )
            ...
            if not self.arguments.quiet:
                for n in beam:
                    ...
                    if not n.finished():
                        p = Sequence(p)
                    print(
                        "(x%d) Program in beam (%f):\n%s"
                        % (n.count, n.logLikelihood, str(p))
                    )
    ...
    def consolidateIdenticalParticles(self, particles):
        ...
        if not self.arguments.quiet:
            print(
                "Consolidated %d particles into %d particles in %f seconds."
                % (startingParticles, len(finalParticles), time() - startTime)
            )
    ...
    def saveParticles(self, finishedPrograms, parseDirectory, targetImage):
        ...
        if self.arguments.distance:
            finishedPrograms.sort(
                key=lambda n: likelihoodCoefficient * n.logLikelihood
                + distanceCoefficient[0] * n.distance[0]
                + distanceCoefficient[1] * n.distance[1]
                + priorCoefficient * n.program.logPrior(),
                reverse=True,
            )
        else:
            finishedPrograms.sort(
                key=lambda n: n.logLikelihood - 0.1 * n.distance, reverse=True
            )
    ...
    def evaluateAccuracy(self):
        ...
        random.seed(42)
        targetPrograms = [
            randomScene(36)() for _ in range(self.arguments.numberOfExamples)
        ]
        ...
        for targetProgram in targetPrograms:
            ...
            if self.arguments.LSTM:
                maximumLength = len(
                    self.recognizer.decoder.targetsOfProgram(targetProgram)
                )
                ...
                print(maximumLength)
                print()
                particles = self.recognizer.beam(
                    None, targetImage, arguments.beamWidth, maximumLength=maximumLength
                )
                ...
                particles = [
                    Particle(program=program, logLikelihood=ll)
                    for ll, program in particles
                ]
            else:
                particles = self.SMC(
                    targetImage, beamSize=arguments.beamWidth, beamLength=k + 1
                )
            ...
            intersectionDistance[k].append(
                len(targetSet & preferredSet) / float(len(targetSet | preferredSet))
            )
        ...
        programDistances = [
            r for rs in list(programDistance.values()) for r in rs if r != None
        ]
        intersectionDistances = [
            r for rs in list(intersectionDistance.values()) for r in rs if r != None
        ]
        print("Got the correct program %d/%d times" % (len(ranks), n))
        print(
            "Average program distance: %f"
            % (sum(programDistances) / float(len(programDistances)))
        )
        print(
            "Average intersection distance: %f"
            % (sum(intersectionDistances) / float(len(intersectionDistances)))
        )
...
def handleTest(a):
    ...
    if l == 0:
        try:
            ...
            pass
    particles = model.SMC(targetImage, beamSize=arguments.beamWidth, beamLength=l)
...
def illustrateContinuous(model):
    ...
    drawingsMatrix = []
    for j in range(100):
        if not os.path.exists("continuousParses/drawings/expert-%d-parses/0.png" % j):
            continue
        drawingsMatrix.append(
            [
                1 - loadImage("continuousParses/drawings/expert-%d.png" % j),
    ...
    for targetProgram in targetPrograms:
        ...
        if False:
            ...
            particles = sorted(particles, key=lambda p: p.logLikelihood, reverse=True)[
                :topK
            ]
            particles = [p.sequence() for p in particles]
        else:
            particles = [
                model.sample(targetImage, maximumLength=len(targetProgram) + 1)
                for _ in range(topK)
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="training and evaluation of recognition models"
    )
    ...
    arguments.trainingData = (
        "syntheticContinuousTrainingData.tar"
        if arguments.continuous
        else "syntheticTrainingData.tar"
    )
    ...
    if arguments.task == "showSynthetic":
        ...
        if arguments.cores == 1:
            ...
            gt = Pool(arguments.cores).map(
                handleTest, [(f, arguments, model) for f in fs]
            )
        print(
            "Got a ground truth parse correct %f"
            % (float(len([None for g, _ in gt if g])) / float(len(gt)))
        )
        ...
        if arguments.distance:
            DistanceModel(arguments).train(
                arguments.numberOfExamples, restore=arguments.r
            )
        else:
            RecognitionModel(arguments).train(
                arguments.numberOfExamples, restore=arguments.r
            )
