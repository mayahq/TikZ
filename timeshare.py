...
def executeTimeshareTasks(
    tasks, dt=1, exponent=1, minimumSlice=0.05, globalTimeout=None
):
    startTime = time.time()
    while len(tasks) > 0:
        sliceStartTime = time.time()

        if globalTimeout != None and time.time() - startTime > globalTimeout:
            break
        ...
        for share, task in zip(shares, tasks):
            ...
            if numberOfActiveSlices == 1:
                share = None
            print("Executing task:", task.arguments[0], "for", share, "sec")
            if share == None:
                print("Blocking until process finishes...")
        ...
        print(
            "Finished giving all of the tasks a slice. Took %f sec, efficiency = %d"
            % (sliceTotalTime, int(100 * dt / sliceTotalTime))
        )
        if exponent > 1:
            dt = dt * exponent
            print("Grew dt to", dt)
...
def executeTimeshareTasksFairly(tasks, dt=1, minimumSlice=0.05, globalTimeout=None):
    ...
    while True:
        ...
        for (
            ...
            print(
                "Executing task:", [str(a) for a in task.arguments], "for", share, "sec"
            )
            result = task.execute(share)
            if result == "still running":
                continue
        ...
        if any(t.finished for t in tasks):
            tasksAndRuntimes = [
                (t, r) for t, r in zip(tasks, totalRunTime) if not t.finished
            ]
...
if __name__ == "__main__":
    ...
    for result in executeTimeshareTasksFairly(
        [
            TimeshareTask(callback, ["high-priority"], logScore=log(0.9)),
            TimeshareTask(callback, ["low-priority"], logScore=log(0.1), timeout=2),
        ],
        dt=1,
        minimumSlice=0.15,
    ):
