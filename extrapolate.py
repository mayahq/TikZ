...
def proposeExtrapolations(programs, N=30):
    ...
    extrapolationGenerators = [
        program.explode().extrapolations() for program in programs
    ]
    for e in interleaveGenerators(extrapolationGenerators):
        ...
        if t.canonicalTranslation() == trace.canonicalTranslation():
            continue
        if any(
            [
                t.canonicalTranslation() == o.canonicalTranslation()
                for _, o in extrapolations
            ]
        ):
            continue
...
def exportExtrapolations(programs, fn, index=None):
    ...
    framedExtrapolations = [
        frameImageNicely(t.draw(adjustCanvasSize=True)) for t in extrapolations
    ]
    if index is not None:
        framedExtrapolations = [
            1 - frameImageNicely(loadImage(index))
        ] + framedExtrapolations
