...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "task",
        choices=["parse", "synthesize", "everything"],
        default="everything",
        type=str,
    )
    parser.add_argument("--particles", "-p", type=int, default=100)
    parser.add_argument("--synthesizeFrom", type=int, default=None)
    parser.add_argument("--programOutputDirectory", type=str, default=None)
    ...
    if arguments.task in ["parse", "everything"]:
        ...
        execute("rm %s/*.png %s/particle*.p" % (parseFileName, parseFileName), "RM")
        execute(
            "python recognitionModel.py test  -t %s -b %d -l 0 --proposalCoefficient 1 --parentCoefficient --distanceCoefficient 5 --distance --mistakePenalty 10 --attention 16 --noisy --quiet "
            % (imageFilename, arguments.particles),
            "NEURALNET",
        )
    ...
    if arguments.task in ["synthesize", "everything"]:
        if arguments.synthesizeFrom is None:
            ...
            eprint(
                "\tLaunching program synthesizer (%d top-ranked empirical parse)"
                % (arguments.synthesizeFrom + 1)
            )
        ...
        if arguments.programOutputDirectory is None:
            ...
            outputDirectory = (
                "--programOutputDirectory %s" % arguments.programOutputDirectory
            )

        execute(
            "python synthesisPolicy.py %s %s %s -f basic --folds 1 --regularize 0.1 --load --evaluate %s "
            % (outputDirectory, extra, timeout, synthesisTarget),
            "SYNTHESIZER",
        )
