...
RUN apt-get install -y --no-install-recommends tk-dev libcairo2-dev
...
RUN apt-get update && \
    ...
    apt-get clean
RUN cd TikZ/sketch-1.7.5/sketch-backend && \
...
RUN pip install https://download.pytorch.org/whl/cpu/torch-0.4.1-cp37-cp37m-win_amd64.whl
