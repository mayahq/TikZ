...
def makeDistanceExamples(targets, programs, continuous=False, reportTime=False):
    ...
    for target, program in zip(targets, programs):
        ...
        for _ in range(10):
            ...
            stuff = program.lines
            if random() < 0.4:
                stuff = np.random.permutation(stuff).tolist()
    ...
    if False:
        for j in range(len(extraCurrent)):
            ...
            showImage(np.concatenate((exampleTargets[j], exampleImages[j]), axis=0))
            print()
            print()
    ...
    if reportTime:
        print(
            "Generated examples from %d programs in %f seconds"
            % (len(programs), time() - startTime)
        )
...
def smoothDistance(p, q, tolerance=0.5):
    def d(a, b):
        ...
        if isinstance(a, Line):
            if a.solid != b.solid or a.arrow != b.arrow:
                return None
    ...
    # adjacency matrix
    adjacency = [[d(a, b) for b in q.lines] for a in p.lines]
    ...
    def minimumCostAlignment(pIndex, availableQ):
        ...
        for (qUsed, mismatchCount, cost) in candidates:
            (recursiveMismatch, recursiveCost) = minimumCostAlignment(
                pIndex + 1, [j for j in availableQ if j != qUsed]
            )
