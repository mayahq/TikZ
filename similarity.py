...
if __name__ == "__main__":
    ...
    jobs = [
        (x, y) for j, x in enumerate(images) for k, y in enumerate(images) if j != k
    ]
    distances = []
    while jobs != []:
        batch = jobs[:100]
        ...
        result = worker.learnedDistances(
            np.array([x for x, _ in batch]), np.array([x for _, x in batch])
        )
        for j in range(len(batch)):
            distances.append(result[j, 1] + result[j, 0])

    matrix = np.zeros((len(images), len(images)))
    indexes = [(x, y) for x in range(len(images)) for y in range(len(images)) if x != y]
    for (x, y), d in zip(indexes, distances):
        matrix[x, y] += d
...
def analyzeFeatures(featureMaps):
    ...
    featureVectors = [
        [featureMaps[k].get(name, 0) for name in featureNames] for k in imageNames
    ]
    ...
    closePrograms = set(
        [
            (n1, n2)
            for j, n1 in enumerate(imageNames)
    ...
    imageOptions = [
        (closeImages, "close in image space"),
    ...
    for programSet, programName in programOptions:
        for imageSet, imageName in imageOptions:
            ...
            indices = np.random.choice(
                list(range(len(overlap))), size=min(100, len(overlap)), replace=False
            )
            ...
            saveMatrixAsImage(
                matrix * 255, "similarity/%s%s.png" % (programName, imageName)
            )
    ...
    for _ in range(10):
        ...
        for j, p1 in enumerate(cp):
            ...
            for k, p2 in enumerate(cp):
                ...
                if difference[0] > 2 * w or difference[1] > 2 * w:
                    continue
        ...
        cp = [
            np.array(
                [
                    min(
                        original[0] + allowedDistance * w,
                        max(original[0] - allowedDistance * w, c[0]),
                    ),
                    min(
                        original[1] + allowedDistance * w,
                        max(original[1] - allowedDistance * w, c[1]),
                    ),
                ]
            )
