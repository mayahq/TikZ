...
def inbounds(p):
    if isinstance(p, tuple):
        return (
            p[0] > 0
            and p[0] < MAXIMUMCOORDINATE
            and p[1] > 0
            and p[1] < MAXIMUMCOORDINATE
        )
...
class AbsolutePoint(Expression):
    ...
    def round(self, p):
        return AbsolutePoint(p * round(self.x / p), p * round(self.y / p))
    ...
    def __eq__(self, o):
        if not isinstance(o, AbsolutePoint):
            return False
    ...
    def noisyEvaluate(self):
        return (
            self.x + truncatedNormal(-1, 1) * COORDINATENOISE,
...
class Label(Program):
    ...
    def __str__(self):
        return 'Label(%s, "%s")' % (self.p, self.c)
    ...
    def mutate(self):
        if random() < 0.5:
            ...
            return Label(
                self.p, choice([l for l in Label.allowedLabels if l != self.c])
            )

    @staticmethod
    def sample():
        return Label(AbsolutePoint.sample(), choice(Label.allowedLabels))
...
class Line(Program):
    ...
    def round(self, p):
        return Line([q.round(p) for q in self.points], self.arrow, self.solid)
    ...
    def draw(self, context):
        ...
        if self.arrow and self.points[0] != self.points[1]:
            ...
            ).normalized() * 0.5 + self.points[1]
            wings = (
                self.points[1] - self.points[0]
            ).rotateNinetyDegrees().normalized() * 0.3
            k1 = retreat + wings
            k2 = retreat - wings
    ...
    def logPrior(self):
        return -math.log(14 * 14 * 14 * 14 * 2 * 2)

    def isDiagonal(self):
        return not (
            len(set(self.usedXCoordinates())) == 1
            or len(set(self.usedYCoordinates())) == 1
        )
    ...
    def __sub__(self, o):
        ...
        dx = sum(
            [
                abs(x1 - x2)
                for x1, x2 in zip(o.usedXCoordinates(), self.usedXCoordinates())
            ]
        )
        dy = sum(
            [
                abs(x1 - x2)
                for x1, x2 in zip(o.usedYCoordinates(), self.usedYCoordinates())
            ]
        )
        return dx + dy
    ...
    def intersects(self, o):
        ...
        if isinstance(o, Line):
            ...
            if (
                len(set(self.usedXCoordinates())) != len(set(o.usedXCoordinates()))
                or len(set(self.usedYCoordinates())) != len(set(o.usedYCoordinates()))
                or self.angle() != o.angle()
            ):
                o = o.epsilonShrink()
                s = self.epsilonShrink()
            return linesIntersect(
                AbsolutePoint(s.points[0].x, s.points[0].y),
    ...
    def lineCommand(points, arrow, solid, noisy=False):
        ...
        if arrow:
            ...
            if noisy:
                style = choice(differentStyles)
            else:
                style = differentStyles[0]
        ...
        if not solid:
            if not noisy:
                ...
                attributes += [
                    ...
                    % (choice(list(range(5))) + 2, choice(list(range(5))) + 2)
                ]
        if noisy:
    ...
    def evaluate(self):
        return [
            Line.lineCommand(
                [p.evaluate() for p in self.points], self.arrow, self.solid
            )
        ]
    ...
    def absolute(x1, y1, x2, y2, arrow=False, solid=True):
        return Line(
            [AbsolutePoint(x1, y1), AbsolutePoint(x2, y2)], arrow=arrow, solid=solid
        )
    ...
    def usedCoordinates(self):
        return set([self.points[0].x, self.points[1].x]), set(
...
class Rectangle(Program):
    ...
    def reflect(self, a, c):
        ...
        return Rectangle.absolute(min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2))
    ...
    def draw(self, context):
        ...
        context.rectangle(
            self.p1.x * 16,
            self.p1.y * 16,
            (self.p2.x - self.p1.x) * 16,
            (self.p2.y - self.p1.y) * 16,
        )
        context.stroke()
    ...
    def translate(self, x, y):
        return Rectangle(self.p1.translate(x, y), self.p2.translate(x, y))

    @staticmethod
    def absolute(x1, y1, x2, y2):
        return Rectangle(AbsolutePoint((x1), (y1)), AbsolutePoint((x2), (y2)))
    ...
    def constituentLines(self):
        return [
            Line([self.p1, AbsolutePoint(self.p2.x, self.p1.y)]),
    ...
    def attachmentPoints(self):
        # all of the edges
        ps = [
            (x, self.p1.y, "v") for x in range(int(self.p1.x + 0.5) + 1, int(self.p2.x))
        ]
        ps += [
            (self.p2.x, y, "h") for y in range(int(self.p1.y + 0.5) + 1, int(self.p2.y))
        ]
        ps += [
            (x, self.p2.y, "v") for x in range(int(self.p1.x + 0.5) + 1, int(self.p2.x))
        ]
        ps += [
            (self.p1.x, y, "h") for y in range(int(self.p1.y + 0.5) + 1, int(self.p2.y))
        ]
        return ps
    ...
    def intersects(self, o):
        if isinstance(o, Circle) or isinstance(o, Label):
            return o.intersects(self)
        if isinstance(o, Line):
    ...
    def evaluate(self):
        return [Rectangle.command(self.p1.evaluate(), self.p2.evaluate())]
    ...
    def mutate(self):
        ...
        if (
            ...
            and dx % 2 == 0
            and random() < 0.5
            and (dx == 2 or (not NIPSPRIMITIVES()))
        ):
            return Circle(AbsolutePoint(self.p1.x + dx / 2, self.p1.y + dy / 2), dx / 2)
        while True:
            p1 = self.p1
            p2 = self.p2
            if random() > 0.5:
...
class Circle(Program):
    ...
    def round(self, p):
        return Circle(self.center.round(p), round(self.radius / p) * p)
    ...
    def draw(self, context):
        ...
        context.set_source_rgb(256, 256, 256)
        context.arc(
            self.center.x * 16, self.center.y * 16, self.radius * 16, 0, 2 * math.pi
        )
        context.stroke()

    def translate(self, x, y):
        return Circle(self.center.translate(x, y), self.radius)

    @staticmethod
    def absolute(x, y):
        return Circle(AbsolutePoint((x), (y)), (1))

    def logPrior(self):
        return -math.log(14 * 14)
    ...
    def attachmentPoints(self):
        ...
        x = self.center.x
        y = self.center.y
        return [(x + r, y, "h"), (x - r, y, "h"), (x, y + r, "v"), (x, y - r, "v")]

    def usedXCoordinates(self):
        return [self.center.x, self.center.x + self.radius, self.center.x - self.radius]

    def usedYCoordinates(self):
        return [self.center.y, self.center.y + self.radius, self.center.y - self.radius]
        ...
        while True:
            if random() < 0.5 or NIPSPRIMITIVES():
                ...
                if self.radius < 2:
                    ...
                    r = self.radius + randomRadiusPerturbation()
    ...
    def inbounds(self):
        return (
            inbounds(self.center.x + self.radius)
            and inbounds(self.center.x - self.radius)
            and inbounds(self.center.y + self.radius)
            and inbounds(self.center.y - self.radius)
        )
    ...
    def evaluate(self):
        return [Circle.command(self.center.evaluate(), self.radius)]
...
class Sequence(Program):
    ...
    def logPrior(self):
        return sum([l.logPrior() for l in self.lines]) - (
            len(self.lines) + 1
        ) * math.log(4)
    ...
    def removeDuplicates(self):
        return Sequence(
            [
                ...
                if not (str(l) in list(map(str, self.lines[:j])))
            ]
        )
    ...
    def onlyOneKindOfObject(self):
        return (
            all(isinstance(l, Line) for l in self.lines)
            or all(isinstance(l, Rectangle) for l in self.lines)
            or all(isinstance(l, Circle) for l in self.lines)
        )
    ...
    def haveUnattachedLines(self):
        linePoints = set(
            [(p.x, p.y) for l in self.lines if isinstance(l, Line) for p in l.points]
        )
        attachmentPoints = set(
            [
                (x, y)
                for l in self.lines
    ...
    def haveOrphanLines(self):
        linePoints = [
            {(p.x, p.y) for p in l.points} for l in self.lines if isinstance(l, Line)
        ]
        for j, ps in enumerate(linePoints):
            others = [pp for i, pp in enumerate(linePoints) if i != j]
            if all(len(ps & pp) == 0 for pp in others):
                return True
        return False
    ...
    def haveOrphanCircles(self):
        ...
        for x in self.lines:
            if not isinstance(x, Circle):
                continue
            a = [(x_, y_) for (x_, y_, _) in x.attachmentPoints()]
            if len(set(a) & linePoints) == 0:
                return True
        return False
    ...
    def extent(self):
        ...
        x1 = (
            max(
                [x for l in parse.lines for x in l.usedXCoordinates()]
                + [MAXIMUMCOORDINATE - 1]
            )
            + 1
        )
        y1 = (
            max(
                [y for l in parse.lines for y in l.usedYCoordinates()]
                + [MAXIMUMCOORDINATE - 1]
            )
            + 1
        )
    ...
    def framedRendering(self, reference=None):
        ...
        return render([self.TikZ()], yieldsPixels=True, canvas=(x1, y1), x0y0=(x0, y0))[
            0
        ]
        ...
        for p in self.lines[:-1]:
            for q in self.lines[1:]:
                ...
                if isinstance(p, Line) and isinstance(q, Line):
                    if p.solid != q.solid or p.arrow != q.arrow:
                        continue
        ...
        for p in self.lines[:-1]:
            for q in self.lines[1:]:
                if p == q:
                    continue
                if isinstance(p, Circle) and isinstance(q, Circle):
                    vectors.append((p.center.x - q.center.x, p.center.y - q.center.y))
                if isinstance(p, Rectangle) and isinstance(q, Rectangle):
                    vectors.append((p.p1.x - q.p1.x, p.p1.y - q.p1.y))
                    vectors.append((p.p2.x - q.p2.x, p.p2.y - q.p2.y))
                if isinstance(p, Line) and isinstance(q, Line):
                    if p.solid != q.solid or p.arrow != q.arrow:
                        continue
                    vectors.append(
                        (p.points[0].x - q.points[0].x, p.points[0].y - q.points[0].y)
                    )
                    vectors.append(
                        (p.points[1].x - q.points[1].x, p.points[1].y - q.points[1].y)
                    )
        return vectors
